...
def get_job(job_id: str) -> Optional[Job]:
    ...
    # Reconstruct the Job instance with the GenerateRequest (deserialized from request_json column)
    req_json = row.get('request_json')
    if req_json:
        try:
            req_dict = json.loads(req_json)
            req = GenerateRequest.model_validate(req_dict)
        except Exception as e:
            logger.warning("[%s] Failed to parse request_json from DB: %s", job_id, e)
            video_url = row.get('video_url') or "http://placeholder.url"
            req = GenerateRequest(video_url=video_url)
    else:
        video_url = row.get('video_url') or "http://placeholder.url"
        req = GenerateRequest(video_url=video_url)
    ...
    # Populate its status, stage, progress, error from the DB row
    db_status = row.get('status') or 'queued'
    db_status_lower = db_status.lower()
    ...
    job.message = f"Status: {db_status}"

    db_progress = row.get('progress')
    if db_progress is not None:
        job.progress = float(db_progress) / 100.0
        job.stage_progress = float(db_progress) / 100.0

    job.error = row.get('error')
    if job.error:
        job.message = job.error

    # Query the generated_shorts table for any clips associated with the job's source_video_id to populate job.clips
    source_video_id = row.get('source_video_id')
    if source_video_id:
        db_clips = db.get_shorts_by_source_video_id(source_video_id)
        clips = []
        for crow in db_clips:
            idx = crow.get('clip_index', 0)
            title = crow.get('title') or f"Clip {idx + 1}"
            
            # Derive start/end
            start = 0.0
            end = 0.0
            ep_json = crow.get('edit_plan_json')
            if ep_json:
                try:
                    ep = json.loads(ep_json)
                    if 'cuts' in ep and ep['cuts']:
                        start = ep['cuts'][0].get('start_time', 0.0)
                        duration = sum(float(c.get('end_time', 0.0)) - float(c.get('start_time', 0.0)) for c in ep['cuts'])
                        end = start + duration
                except Exception:
                    pass
            
            clip_id = crow.get('id') or f"shorts_{job_id}_{idx}"
            url = crow.get('file_path') or f"/clips/{clip_id}/{idx}.mp4"
            clips.append({
                "index": idx,
                "title": title,
                "start": round(start, 2),
                ...
                "filename": download_filename(title, req.language, idx)
            })
        job.clips = clips
...
def _run_pipeline(job: Job) -> None:
    ...
    try:
        ...
        if req.edit_plan:
            ...
            edit_plan = EditPlan.model_validate(req.edit_plan)
            source_duration = _get_duration(source_mp4)
            ...
            job.set_stage("selecting", 1.0, "Using custom AI edit plan.")
        else:
        ...
        for index, win in enumerate(windows):
            ...
            if req.edit_plan:
                from .edit_plan import EditPlan
                from .plan_executor import map_transcript_to_composed
                from .transcriber import _get_duration
