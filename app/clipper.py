...
from .paths import CLIPS_DIR, FONTS_DIR, MASKS_DIR, ASSETS_DIR
...
class ClipOptions:
    ...
    edit_plan: Optional[Any] = None    # optional EditPlan schema
...
def _music_audio_graph_dynamic(voice_label: str, mus_idx: int, volume: float, duck: float = 70.0) -> str:
    """Filtergraph mixing background track under a dynamic voice label."""
    ...
    return (
        f"[{voice_label}]asplit=2[__v1][__v2];"
...
def _music_audio_graph(mus_idx: int, volume: float, duck: float = 70.0) -> str:
    """Standard music mixing over 0:a for backward compatibility."""
    return _music_audio_graph_dynamic("0:a", mus_idx, volume, duck)
...
def generate_clip(source_mp4: Path, start: float, end: float, opts: ClipOptions) -> Path:
    ...
    edit_plan = opts.edit_plan

    if edit_plan:
        from .plan_executor import build_composition_filter, get_composed_duration
        from .transcriber import _get_duration
        
        source_duration = _get_duration(Path(src))
        duration = get_composed_duration(edit_plan, source_duration)
        width, height = target_size(opts.aspect_ratio, opts.fit_mode)
        has_mask = (opts.fit_mode == FitMode.SQUARE)
        
        sfx_paths = []
        if edit_plan.sound_effects:
            # Create sound_effects assets dir if missing
            (ASSETS_DIR / "sound_effects").mkdir(parents=True, exist_ok=True)
            for sfx in edit_plan.sound_effects:
                sfx_path = ASSETS_DIR / "sound_effects" / sfx.name
                if sfx_path.is_file():
                    sfx_paths.append(str(sfx_path.resolve()))
                else:
                    logger.warning("Sound effect file not found on disk, skipping: %s", sfx_path)
                
        comp_fc, comp_v_out, comp_a_out = build_composition_filter(
            edit_plan=edit_plan,
        ...
        if has_mask:
            ...
            inputs = ["-i", src, "-loop", "1", "-i", str(mask.resolve())]
            music_idx = 2 + len(sfx_paths)
            tail = ["-shortest"]
        else:
            inputs = ["-i", src]
            music_idx = 1 + len(sfx_paths)
            tail = []
        ...
        if opts.fit_mode == FitMode.SQUARE:
            ...
            if opts.bar_text and opts.bar_text.strip():
                ...
                for li, ln in enumerate(lines):
                    ...
                    y = (f"'{base_y}+40*(1-min(t/0.45\\,1))'" if anim == "slide" else str(base_y))
                    stages.append(
                        f"[{last}]drawtext=fontfile={_rel_for_filter(_BAR_FONT, out_dir)}:"
                        f"text='{_escape_drawtext(ln)}':"
                        f"fontcolor={col}:fontsize={font_size}:x=(w-text_w)/2:y={y}{alpha_expr}:"
                        f"borderw=3:bordercolor=black@0.85[ttl{li}]"
                    )
                    last = f"ttl{li}"
            sig_stages, last = _signature_stages(last, opts.signature, w, h, out_dir)
            stages += sig_stages
            stages.append(_caption_stage(last, opts, out_dir))
            
            fc = comp_fc + ";" + ";".join(stages)
        else:
            stages = _finish_stages(comp_v_out, width, height, opts, out_dir)
            fc = comp_fc + ";" + ";".join(stages)
        ...
        if has_music:
            ...
            fc = fc + ";" + _music_audio_graph_dynamic(comp_a_out, music_idx, opts.music_volume, opts.music_duck)
            audio_map = ["-map", "[aout]"]
        else:
            audio_map = ["-map", f"[{comp_a_out}]"]
    else:
